...
barrier_check(void)
{
    ...
    if (barrier_count < barrier_threshold) {
        //Puts the process to sleep, releasing the lock internally until it’s woken up by the last process.
        // (void*)&barrier_count acts as the channel, so all sleepers share the same wakeup point.
